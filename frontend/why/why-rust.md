...
Java's cost problem is the reason Eco exists at all in one sense: the old Spring Boot auth service was a heavyweight general-purpose stack for what should be the lightest, most reusable domain in the estate. Rewriting it in Rust corrected both the cost and the domain boundary. **The Assessment estate — the last Java holdout, whose `assessment-backend` was a full Spring Boot 3.2 / Java 17 service — has been completely converted to Rust** (verified live on staging then shipped to production on 10 Aug 2026). An identical-workload head-to-head load test on the same hardware measured the Rust backend at 1,583 req/s vs the Java baseline's 1,335 req/s (+19%), with p95 latency dropping from 58.6 ms to 15.0 ms (−74%) and service memory from ~312 MB to ~11 MB. The assessment estate is now entirely Rust.
