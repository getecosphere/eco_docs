...
The most common question about Ecosphere is: *why not just use Docker?* The short answer: because Ecosphere targets a different — and for many teams, better — point on the isolation-vs-cost curve.

## Why Ecosphere does not use containers
...
This matters far more than it used to. When an AI agent (or a vibecoder relying on agents) iterates on a project, it rebuilds, restarts, and probes services constantly. Each cycle with containers means re-inflating a filesystem, re-running image pulls, and waiting on overlay mounts. Ecosphere runs services as **native processes** — the agent edits, restarts, and gets feedback in seconds, not minutes. For teams whose daily driver is an AI assistant, per-cycle cost is the metric that decides the workflow.

**3. The isolation Docker provides is the wrong granularity for most apps.**

Containers isolate *per service*. But what most organizations actually want to isolate is *per application*. Ecosphere moves isolation up to the Proxmox CT boundary — one container (CT) per estate/application — and runs services natively inside it. You get application-level isolation with none of the per-service overhead.

**4. Ecosphere is deliberately young.**

Ecosphere was first committed to on **29 June 2026**. From day one it was designed around host-native operations for a Proxmox base — not as a competitor to Docker, but as the deliberate absence of the container layer. Its "compose" is `ecompose.yml` + `eco up`; its "image" is a provisioned CT template; its "container runtime" is the Proxmox LXC/CT runtime itself.

## Head-to-head comparison

| Concern | Docker Compose | Ecosphere |
| --- | --- | --- |
...
If you already run Kubernetes, need hard per-service resource isolation, or rely on a container-specific marketplace of images, Docker/K8s is the right tool. Ecosphere is a different philosophy: **native processes, application-scale isolation, on Proxmox** — built for organizations that want to run many composed applications predictably and cheaply, especially with AI in the loop.
